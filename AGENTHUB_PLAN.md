...
4. 运行最小有效验证。
5. 更新本计划的课程状态和验收记录。
6. 使用 `.agents/skills/agenthub-learning-checkpoint/SKILL.md` 完成 Git 检查点。
...
| 0.2 | 配置管理 | YAML 配置、环境变量覆盖、配置加载器 | ⬜ 待开始 |
| 0.3 | 结构化日志 | 统一日志组件、日志级别与字段 | ⬜ 待开始 |
| 0.4 | 错误处理 | 错误包装、错误类型、错误码 | ⬜ 待开始 |
| 0.5 | 工程命令 | Makefile、格式化、检查、构建、运行 | ⬜ 待开始 |
| 0.6 | 测试基础 | 单元测试、表驱动测试、覆盖率 | ⬜ 待开始 |

### Phase 1：Agent Runtime

| 课程 | 内容 | 交付物 | 状态 |
|---|---|---|---|
| 1.1 | Model 与 Message 抽象 | 模型接口和消息模型 | ⬜ 待开始 |
| 1.2 | Tool 系统 | Tool 接口、Registry、参数校验 | ⬜ 待开始 |
...
- 当前课程：**0.2 配置管理**
- 上一课程：**0.1 项目结构与 Go Module，已完成**
- 下一验收目标：应用能从 YAML 读取名称、环境和服务端口，并允许环境变量覆盖
