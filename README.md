...
当前 CLI 支持连续多轮输入和 `/tools` 工具目录命令，并同时接入本地工具与多个真实 MCP Server：受限的 `read_project_file` 可按需读取项目根目录内的常见 UTF-8 文本文件，绝对路径、目录穿越、符号链接逃逸、`.env` 和超过 64 KiB 的文件会被拒绝；`calculator` 与 `accounting` 两个独立 stdio MCP Server 都提供远程 `add_numbers`，AgentHub 通过 `calculator__add_numbers` 和 `accounting__add_numbers` 命名空间避免同名冲突。MCP Manager 根据可读的 Server 配置建立多个 Session，Eino Adapter 负责工具发现与远程调用，工具目录统一展示工具来源、所属 Server、启用状态、描述与参数 Schema。HTTP/SSE 和持久化将在后续课程实现。详细路线见 [`AGENTHUB_PLAN.md`](AGENTHUB_PLAN.md)。
...
- `internal/`：持续演进的私有生产代码，目前包含会话上下文、工具目录、多 MCP Session Manager、命名空间工具代理与本地 stdio MCP Server；历史自研 Runtime 已迁入 `examples/selfbuilt-runtime/internal/`。
